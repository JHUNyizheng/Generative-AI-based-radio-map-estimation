...
class part2all_brt_dataloader(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_gain = np.reshape(image_gain, [64,64])
        image_gain_mask = np.reshape(image_gain_mask, [1,64,64])

        # inputs to radioUNet
        outputs = np.stack([image_buildings , image_Tx, image_gain], axis=0)

        return [image_gain_mask,outputs]
