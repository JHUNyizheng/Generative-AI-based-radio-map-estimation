...
def sample_mask(mask, sample_rate):
    ...
    b[id[random_id][:, 0], id[random_id][:, 1]] = 1

    return b

class RadioMap3D(Dataset):
    """
    ...
    def __init__(self, path, phase):
        ...
        self.dir_dataset = '/data/RadioUnet/RadioMap3DSeer/'
        ...
        self.numTx = 80
        self.thresh = 0
        self.dir_gain = self.dir_dataset + "gain/"  # RSRP的地址
        self.dir_buildings = self.dir_dataset + "png/buildingsWHeight/" # building的地址
        self.dir_Tx = self.dir_dataset + "png/antennasWHeight/" # 基站位置的地址
    ...
    def __getitem__(self, index):
        ...
        #
        image_buildings = np.reshape(image_buildings, [256, 256]) / 255
        image_gain = np.reshape(image_gain, [256, 256]) / 255
        image_Tx = np.reshape(image_Tx, [256, 256]) / 255
        #
        ...
        if self.phase == 'train':
            ...
            mask = 1 - image_buildings
            mask[mask != 1] = 0
            s_mask = sample_mask(mask, sample_rate=sample_rate)
        else:
            mask = 1 - image_buildings
            mask[mask != 1] = 0
            s_mask = sample_mask(mask, sample_rate=self.sample_rate)
